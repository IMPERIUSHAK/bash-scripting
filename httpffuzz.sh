#!/bin/bash

cleanup(){
    echo -e "\033[1;33m Usage[!] CTRL-C detected, stopping scan ...\033[0m"
    kill 0
    exit 1
}

worker(){

    local word="$1"

    response=$(curl -k -s \
    -o /dev/null \
    --connect-timeout "$TIMEOUT" \
    -A "$UA" \
    -w "%{http_code}|%{size_download}|%{content_type}"\
    "$url/$word"
    )

    code=$(echo "$response" | cut -d'|' -f1)
    size=$(echo "$response" | cut -d'|' -f2)

    if [[ "$code" -ne 404 && "$code" -ne 0 && "$size" != "$main_size" ]]; then
        echo -e "\033[1;32m[$code]\033[0m $url/$word"
    fi
}

trap cleanup INT

ulr=""
wordls=""

if [[ $# -lt 4 ]]; then
    echo -e "\033[1;31m[!] Incorrect number of arguments\033[0m"
    echo -e "\033[1;33m Usage: $0 -w [wordlist] - u [URL]\033[0m"
    exit 1
fi

if [[ "$1" == "-u" ]] && [[ "$3" == "-w"  ]]; then
        url="$2"
        wordls="$4"
elif [[ "$1" == "-w" ]] && [[ "$3" == "-u" ]]; 
then
        url="$4"
        wordls="$2"

else
        echo -e "\033[1;33m Incorrect usage of script:\n httpffuz -w [wordlist] -u [Url]\033[0m"
        exit 0
fi


THREADS=20
TIMEOUT=2

curr=0
lns=$(wc -l < "$wordls")
main_size=$(curl -k -s \
    -o /dev/null \
    --connect-timeout "$TIMEOUT" \
    -A "$UA" \
    -w "%{size_download}"\
    "$url/$word"
)

while IFS= read -r word; do
    ((curr++))

    worker "$word" &
    
    echo -ne "$curr/$lns\r"

    while [ $(jobs -r | wc -l) -ge "$THREADS" ]; do
        sleep 0.1
    done

done < "$wordls"
wait
echo >&2
